    ...
    try {
      ...
      if (response && response.length > 0) {
        ...
        if (!currentOrg) {
          ...
          localStorage.setItem('currentOrganization', currentOrg.organizationId._id);
        }
        ...
        const [projectsResponse, statsResponse] = await Promise.all([
          ...
          ApiService.getStats().catch(() => ({ stats: {} }))
        ]);
      ...
      if (error.message?.includes('token') || error.message?.includes('unauthorized') || error.status === 401) {
